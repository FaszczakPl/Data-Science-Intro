...
max = np.max(playway_values[:,:-1],axis=1)
min = np.min(playway_values[:,:-1],axis=1)
max_min_diff = max - min
max_min_diff = np.ndarray.reshape(max_min_diff,(22,-1))
playway_values = np.concatenate((playway_values,max_min_diff),axis=1)
print(playway_values)
np.savetxt("playway.txt",playway_values)
